...
(define (handlers . l)
  (make-hash (map (lambda (y) 
                    (cons (handler-status y) y)) l)))
...
(define (handle-error code msg)
  (handle code msg (lambda (x) 
                     (error 
                      (string-append 
                       "Error: " msg ". Server returned " code ":" 
                       (neo4j-response-body x))))))
...
(define neo4j-request-headers 
  (list "Accept:application/json"))
(define neo4j-post-headers 
...
(define (handle-response r handler-hash)          
  (let ([respcode (neo4j-response-status r)])
    (cond [(hash-has-key? handler-hash respcode)            
          ...
          [else (string-append 
                 "Response handler not defined for status:" 
                 (neo4j-response-header r) 
                 respcode)])))
...
(define (neo4j-post n4j surl reqbody handler-hash)  
  (let* 
      ([url (string->url surl)]
       [resp (read-response 
              (post-impure-port url reqbody neo4j-post-headers) "" )])    
...
(define (neo4j-put n4j surl reqbody handler-hash)
  (let* 
      ([url (string->url surl)]
       [resp (read-response-head-only 
              (put-impure-port url reqbody neo4j-post-headers) "" )])    
    (handle-response resp handler-hash)))
...
(define (neo4j-delete n4j surl handler-hash)    
  (let* 
      ([url (string->url surl)]
       [resp (read-response-head-only 
              (delete-impure-port url neo4j-request-headers) "" )])     
    (handle-response resp handler-hash)      
    ))
...
(define (neo4j-init baseurl)  
  (with-handlers ([(lambda (v) 
                     (begin (display v) #t)) 
                   (lambda (v) "Failed to connect!")])    
    (let* ([n4j (neo4j-server 
                 baseurl "" "" "" "" "" "" "" default-response-handlers)]
...
(define (get-neo4j-root n4j)
  (read-response 
   (neo4j-request-port-raw n4j "/") ""))


(define (get-node n4j nodeid)
  (let* ([snodeid (nodeid->string nodeid)]
         [url (string-append 
               (neo4j-server-node n4j) "/" snodeid)])
    (neo4j-get n4j url 
               (response-handlers-get-node 
...
(define (create-node n4j [props #f])
  (let* ([url (neo4j-server-node n4j)]
    ...
    (neo4j-post n4j url reqbody 
                (response-handlers-create-node 
                 (neo4j-server-handlers n4j)))))

(define (set-node-props n4j nodeid [props #f])
  (let* ([snodeid (nodeid->string nodeid)]         
         [url (string-append 
               (neo4j-server-node n4j) "/" snodeid "/properties")]         
         [reqbody (string->bytes/locale 
                   (if (eq? props #f) ""
                       (jsexpr->json props)))])
    (neo4j-put n4j url reqbody 
               (response-handlers-set-node-properties 
                (neo4j-server-handlers n4j)))))
...
(define (get-node-props n4j nodeid)
  (let* ([snodeid (nodeid->string nodeid)]
         [url (string-append 
               (neo4j-server-node n4j) "/" snodeid "/properties")])
    (neo4j-get n4j url 
               (response-handlers-get-node-properties 
                (neo4j-server-handlers n4j)))))
...
(define (remove-node-props n4j nodeid)
  (let* ([snodeid (nodeid->string nodeid)]
         [url (string-append 
               (neo4j-server-node n4j) "/" snodeid "/properties")])
    (neo4j-delete n4j url 
                  (response-handlers-remove-node-properties 
                   (neo4j-server-handlers n4j)))))
...
(define (set-node-prop n4j nodeid prop value)
  (let* ([snodeid (nodeid->string nodeid)]         
         [url (string-append 
               (neo4j-server-node n4j) "/" snodeid "/properties/" prop)]                  
         [reqbody (string->bytes/locale (jsexpr->json value))])
    (neo4j-put n4j url reqbody 
               (response-handlers-set-node-property 
                (neo4j-server-handlers n4j)))))
...
(define (get-node-prop n4j nodeid prop)
  (let* ([snodeid (nodeid->string nodeid)]
         [url (string-append 
               (neo4j-server-node n4j) "/" snodeid "/properties/" prop)])
    (neo4j-get n4j url 
               (response-handlers-get-node-property 
                (neo4j-server-handlers n4j)))))
...
(define (remove-node-prop n4j nodeid prop)
  (let* ([snodeid (nodeid->string nodeid)]
         [url (string-append 
               (neo4j-server-node n4j) "/" snodeid "/properties/" prop)])
    (neo4j-delete n4j url 
                  (response-handlers-remove-node-property 
                   (neo4j-server-handlers n4j)))))
...
(define (create-relationship n4j relfrom relto reltype [reldata (hash)])
  (let* (
         ...
         [reltostr (string-append 
                    (neo4j-server-node n4j) "/node/" reltoid )]                             
         [d (hash 'to reltostr 'type reltype 'data reldata)]         
         [url (string-append 
               (neo4j-server-node n4j) "/" relfromid "/relationships" )]
         [reqbody (string->bytes/locale (jsexpr->json d))])    
    (neo4j-post n4j url reqbody 
                (response-handlers-create-relationship 
                 (neo4j-server-handlers n4j)))))
...
(define (get-rel-props n4j relid)
  (let* ([srelid (nodeid->string relid)]         
         [url (string-append 
               (neo4j-server-baseurl n4j) "/relationship/" relid "/properties")])
    (begin 
      (display url)      
      (neo4j-get n4j url 
                 (response-handlers-get-relationship-properties 
                  (neo4j-server-handlers n4j))))))
...
(define (set-rel-props n4j relid [props #f])
  (let* ([srelid (nodeid->string relid)]         
         [url (string-append 
               (neo4j-server-baseurl n4j) "/relationship/" srelid "/properties")]         
         [reqbody (string->bytes/locale 
                   (if (eq? props #f) ""
                       (jsexpr->json props)))])
    (neo4j-put n4j url reqbody 
               (response-handlers-set-relationship-properties 
                (neo4j-server-handlers n4j)))))
...
(define (remove-rel-props n4j relid)
  (let* ([srelid (nodeid->string relid)]
         [url (string-append 
               (neo4j-server-baseurl n4j) "/relationship/" srelid "/properties")])
    (neo4j-delete n4j url 
                  (response-handlers-remove-relationship-properties 
                   (neo4j-server-handlers n4j)))))
...
(define (get-rel-prop n4j relid prop)
  (let* ([srelid (nodeid->string relid)]
         [url (string-append 
               (neo4j-server-baseurl n4j) "/relationship/" srelid "/properties/" prop)])
    (neo4j-get n4j url 
               (response-handlers-get-relationship-property 
                (neo4j-server-handlers n4j)))))
...
(define (set-rel-prop n4j relid prop value)
  (let* ([srelid (nodeid->string relid)]         
         ...
         [reqbody (string->bytes/locale (jsexpr->json value))])
    (neo4j-put n4j url reqbody 
               (response-handlers-set-relationship-property 
                (neo4j-server-handlers n4j)))))
...
(define (remove-rel-prop n4j relid prop)
  (let* ([srelid (nodeid->string relid)]
         [url (string-append 
               (neo4j-server-baseurl n4j) "/relationship/" srelid "/properties/" prop)])
    (neo4j-delete n4j url 
                  (response-handlers-remove-relationship-property 
                   (neo4j-server-handlers n4j)))))
...
(define (delete-rel n4j relid)
  (let* ([srelid (nodeid->string relid)]
         [url (string-append 
               (neo4j-server-baseurl n4j) "/relationship/" srelid)])
    (neo4j-delete n4j url 
                  (response-handlers-delete-node 
                   (neo4j-server-handlers n4j)))))
...
(define (get-rel-types n4j)
  (let* ([url (string-append 
               (neo4j-server-baseurl n4j) "/relationship/types")])
    (neo4j-get n4j url 
               (response-handlers-get-relationship-property 
                (neo4j-server-handlers n4j)))))
...
(define (get-node-rel n4j nodeid inouttype reltypes)
  (let* ([snodeid (nodeid->string nodeid)]
         ...
         [url (string-append 
               (neo4j-server-node n4j) 
               "/" snodeid "/relationships/" inouttype reltypelist)])
    (neo4j-get n4j url 
